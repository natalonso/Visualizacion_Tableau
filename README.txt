
#####################################################################
TÉCNICAS A USAR
#####################################################################

-Filtering: quitar datos para visualizar mejor la parte de interés
-Brushing o interactividad: sobresalta los datos sobre los que quieres hacer hincapié
-Linking: enlazado entre figuras
-Panning: desplazar una imagen (mapa)
-Zooming: zoom

#####################################################################
DIAPOSITIVA 1:
#####################################################################

-Introducción pennsylvania, densidad población, distritos, introducción al proyecto, dto de policia... -> enlace a la página con información sobre distritos.
-Hablar brevemente de que es el EMS y el sistema de emergencia de eeuu.

#####################################################################
DIAPOSITIVA 2:
#####################################################################

-EDA general con:
	-Mapa localizaciones.
	-Series temporales.
	-Nube de palabras con los motivos.
-Justificar que para comprobar la calidad de los datos se ha investigado el pico en llamadas a emergencias del 2 de marzo de 2018: enlace a la noticia.

#####################################################################
DIAPOSITIVA 3: TRAFFIC 
#####################################################################

-Filtro por type emer traffic y vemos que sobresale vehicle accidente.
-Decidimos centrarnos en esta emergencia "vehicle accident" y vemos los distritos por orden de número de llamadas.
-Vemos que sobresales el 19401, brevemente mencionamos el distrito (Norristown) y decimos hace un mapa de calor para ver las franjas horarias donde mayor incidencia hay.

#####################################################################
DIAPOSITIVA 4: FIRE
#####################################################################

#####################################################################
DIAPOSITIVA 5: EMS
#####################################################################

#####################################################################
DIAPOSITIVA 6: DECISIONES FINALES A TRANSMITIR AL DTO POLICIA
#####################################################################